...
class LoggingClient(Service):
    ...
    def start(self):
        self._log("Start called on Logging Client:")
        # only start if not already running
        if not self._running:
            self._log(" Starting Logging Client")
            self._running = True
            self._sender_thread = threading.Thread(target=self._run_sender)
            self._sender_thread.start()
        else:
            self._log(" Logging Client already running")

    # Stops the receiver and processor, option join thread
    def stop(self, join=False):
        self._log(" Stop called on Logging Client")
        # if running, stop and join threads
        if self._running:
            self._log(" Stopping Logging Client")
            self._running = False

            if join:
                self.join()
        else:
            self._log(" Logging Client not running")
    ...
    def _run_sender(self):
        self._log("Logging Client running")
        ...
        self._log("Logging Client exiting")
